...
//    }
    ...
    // Issue #31: Experimental EXE patching for creature speed
    // Apply binary patches to ST.EXE instead of modifying creature data
    if (params.creatureSpeedMultiplier && params.creatureSpeedMultiplier !== 1.0) {
        console.log("");
        console.log("═══════════════════════════════════════════════════════════");
        console.log("Issue #31: Applying EXE patches for creature speed");
        console.log("═══════════════════════════════════════════════════════════");
        
        try {
            const patchExe = require('./patch_exe_speed');
            const patchSuccess = patchExe.patchSTExe(stDir, params.creatureSpeedMultiplier);
            
            if (patchSuccess) {
                console.log("✓ EXE patching completed - ST.EXE modified");
                console.log("  Speed multiplier: " + params.creatureSpeedMultiplier + "x");
                console.log("  Modified executable will be included in final ISO");
            } else {
                console.log("⚠ No EXE patches applied - patterns not found");
                console.log("  Binary may require manual reverse engineering");
            }
        } catch (error) {
            console.log("ERROR applying EXE patches: " + error.message);
